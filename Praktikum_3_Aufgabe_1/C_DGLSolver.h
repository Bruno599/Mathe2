...
public:
	...
	void C_DGLSolver_Euler(double Xstart, double Xend, int steps, CMyVektor y_start)
	{
		...
		for (int i = 0; i < steps; i++)
		{
			...
			std::cout << std::setprecision(8) << "\t y = ( " << y.get_wert(0) << "; " << y.get_wert(1) << ")" << std::endl;

			y_strich = derivation(y, x);

			std::cout << std::setprecision(8) <<"\t y' = ( " << y_strich.get_wert(0) << "; " << y_strich.get_wert(1) << ")" << std::endl;
	...
	void C_DGLSolver_Heun(double Xstart, double Xend, int steps, CMyVektor y_start)
	{
		...
		for (int i = 0; i < steps; i++)
		{
			...
			std::cout << std::setprecision(8) << "\t y'_orig = ( " << y_strich.get_wert(0) << "; " << y_strich.get_wert(1) << ")" << std::endl;

			y_sec = y + (h * y_strich);

			//Heun Test

			std::cout << std::setprecision(8) << "\t y_test = ( " << y_sec.get_wert(0) << "; " << y_sec.get_wert(1) << ")" << std::endl;


			x2 = Xstart + ((i + 1) * h);		//Schrittweite für den Test = doppelte vom Ursprung
			y_strich_test = derivation(y_sec, x2);

			std::cout << std::setprecision(8) << "\t y'_test = ( " << y_strich_test.get_wert(0) << "; " << y_strich_test.get_wert(1) << ")" << std::endl;
			...
			std::cout << std::setprecision(8) << "\t y'_Mitte = ( " << y_strich_mitte.get_wert(0) << "; " << y_strich_mitte.get_wert(1) << ")" << std::endl;
